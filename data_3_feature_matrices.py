# -*- coding: utf-8 -*-
"""
Created on Mon Feb 17 12:23:10 2025
...
#creates the data youll need for 3 feature models 


# clean ordered (0-1) reduced data multiplied by rain 
...
combo=tz2*fz2*sz2
i10_r=np.zeros((tz1*4, 4,combo))
i15_r=np.zeros((tz1*4, 4,combo))
i30_r=np.zeros((tz1*4, 4,combo))
i60_r=np.zeros((tz1*4, 4,combo))
s=0;

#This is the data used in models_3feature.py in oder to work you need to record the marices a:
# i10, i15, i30, i60 respectivly
...
for j in range (tz2):
    for k in range (fz2):
        for l in range (sz2):
            ...
            ver_s=np.concatenate(( s_i10[:,l], s_i10[:,l], s_i10[:,l], s_i10[:,l]))
            ver=np.column_stack((ver_re, ver_t, ver_f,  ver_s))
            ...
            ver_s=np.concatenate(( s_i15[:,l], s_i15[:,l], s_i15[:,l], s_i15[:,l]))
            ver=np.column_stack((ver_re, ver_t, ver_f,  ver_s))
            ...
            ver_s=np.concatenate(( s_i30[:,l], s_i30[:,l], s_i30[:,l], s_i30[:,l]))
            ver=np.column_stack((ver_re, ver_t, ver_f,  ver_s))
            ...
            ver_s=np.concatenate(( s_i60[:,l], s_i60[:,l], s_i60[:,l], s_i60[:,l]))
            ver=np.column_stack((ver_re, ver_t, ver_f,  ver_s))
