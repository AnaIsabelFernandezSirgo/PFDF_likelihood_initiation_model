# -*- coding: utf-8 -*-
"""
Created on Tue Oct 28 07:07:01 2025
...
data_rained=pd.read_excel(r'D:/PFDF_08_2025/three_feature_c.xlsx', sheet_name=[ 'Sheet2','Sheet3','Sheet4' ,'Sheet5','Sheet6','Sheet7'])
...
#set for 2 features mdNBR_1000 and L_2_MH
fire_col=['mdNBR_1000'	,'L_2_MH'	]
...
s_i2=i2_s[soil_col].to_numpy(copy=True)
s_i5= i5_s[soil_col].to_numpy(copy=True)
s_i10= i10_s[soil_col].to_numpy(copy=True)
s_i15= i15_s[soil_col].to_numpy(copy=True)
s_i30= i30_s[soil_col].to_numpy(copy=True)
s_i60= i60_s[soil_col].to_numpy(copy=True)


combos = [(f, t, s) for f, t, s in product(topography_col,fire_col, soil_col)]


number_data,number_terrain=t_i10.shape
number_fire=f_i10.shape[1]
number_soil=s_i10.shape[1]





number_combo=number_fire*number_terrain*number_soil

#CREATING THE MATRICES WITH THE DIFFERENT COMBINATIONS


i2=np.empty((number_data, 6,number_combo))
i5=np.empty((number_data, 6,number_combo))
i10=np.empty((number_data, 6,number_combo))
i15=np.empty((number_data, 6,number_combo))
i30=np.empty((number_data, 6,number_combo))
i60=np.empty((number_data, 6,number_combo))
s=0;




for s, (j, k,q) in enumerate(product(range(number_terrain), range(number_fire),range(number_soil))):
    i2[:, :, s] = np.column_stack((re[:, 0], t_i2[:, j], f_i2[:, k], s_i2[:, q], number, vtype))
    i5[:, :,s] = np.column_stack((re[:,0],t_i5[:,j],f_i5[:,k], s_i5[:, q],number,vtype))
    i10[:, :, s]= np.column_stack((re[:,0],t_i10[:,j],f_i10[:,k], s_i10[:, q],number,vtype))
    i15[:, :, s]= np.column_stack((re[:,0],t_i15[:,j],f_i15[:,k], s_i15[:, q],number,vtype))
    i30[:, :, s]= np.column_stack((re[:,0],t_i30[:,j],f_i30[:,k], s_i30[:, q],number,vtype))
    i60[:, :, s]= np.column_stack((re[:,0],t_i60[:,j],f_i60[:,k], s_i60[:, q],number,vtype))
...
test=np.empty((length_test,6,number_combo,number_permutation,number_kfolds))
train=np.empty((length_train,6,number_combo,number_permutation,number_kfolds));
...
#allocating
lm_fold_ts_k=np.empty((number_combo,number_permutation,number_kfolds ))
lm_fold_tp_k=np.empty((number_combo,number_permutation,number_kfolds))
lm_fold_fn_k=np.empty((number_combo,number_permutation,number_kfolds))
lm_fold_fp_k=np.empty((number_combo,number_permutation,number_kfolds))
lm_fold_tn_k=np.empty((number_combo,number_permutation,number_kfolds))


lm_all_info=np.empty((number_combo, number_permutation, number_kfolds, length_test,6))
lm_info=np.empty((length_test,6))
lm_RandA=np.empty((number_combo,number_permutation,number_kfolds))
lm_pr_auc=np.empty((number_combo,number_permutation,number_kfolds))
lm_roc_auc=np.empty((number_combo,number_permutation,number_kfolds))
lm_ap=np.empty((number_combo,number_permutation,number_kfolds))
lm_ap_one=np.empty((number_combo,number_permutation,number_kfolds))


lm_p_ts_p=np.empty((number_combo,number_permutation))
lm_p_tp_p=np.empty((number_combo,number_permutation))
lm_p_fn_p=np.empty((number_combo,number_permutation))
lm_p_fp_p=np.empty((number_combo,number_permutation))
lm_p_tn_p=np.empty((number_combo,number_permutation))
lm_p_RandA=np.empty((number_combo,number_permutation))
lm_p_pr_auc=np.empty((number_combo,number_permutation))
lm_p_roc_auc=np.empty((number_combo,number_permutation))
lm_p_ap=np.empty((number_combo,number_permutation))
lm_p_ap_one=np.empty((number_combo,number_permutation))

lm_pre=np.empty((length_test,number_combo,number_permutation,number_kfolds ))

#LDA

lda_fold_ts_k=np.empty((number_combo,number_permutation,number_kfolds ))
lda_fold_tp_k=np.empty((number_combo,number_permutation,number_kfolds))
lda_fold_fn_k=np.empty((number_combo,number_permutation,number_kfolds))
lda_fold_fp_k=np.empty((number_combo,number_permutation,number_kfolds))
lda_fold_tn_k=np.empty((number_combo,number_permutation,number_kfolds))

lda_all_info=np.empty((number_combo, number_permutation, number_kfolds, length_test,6))
lda_info=np.empty((length_test,6))
lda_RandA=np.empty((number_combo,number_permutation,number_kfolds))
lda_pr_auc=np.empty((number_combo,number_permutation,number_kfolds))
lda_roc_auc=np.empty((number_combo,number_permutation,number_kfolds))
lda_ap=np.empty((number_combo,number_permutation,number_kfolds))
lda_ap_one=np.empty((number_combo,number_permutation,number_kfolds))


lda_p_ts_p=np.empty((number_combo,number_permutation))
lda_p_tp_p=np.empty((number_combo,number_permutation))
lda_p_fn_p=np.empty((number_combo,number_permutation))
lda_p_fp_p=np.empty((number_combo,number_permutation))
lda_p_tn_p=np.empty((number_combo,number_permutation))
lda_p_RandA=np.empty((number_combo,number_permutation))
lda_p_pr_auc=np.empty((number_combo,number_permutation))
lda_p_roc_auc=np.empty((number_combo,number_permutation))
lda_p_ap=np.empty((number_combo,number_permutation))
lda_p_ap_one=np.empty((number_combo,number_permutation))


lda_pre=np.empty((length_test,number_combo,number_permutation,number_kfolds ))



#RF

rf_fold_ts_k=np.empty((number_combo,number_permutation,number_kfolds ))
rf_fold_tp_k=np.empty((number_combo,number_permutation,number_kfolds))
rf_fold_fn_k=np.empty((number_combo,number_permutation,number_kfolds))
rf_fold_fp_k=np.empty((number_combo,number_permutation,number_kfolds))
rf_fold_tn_k=np.empty((number_combo,number_permutation,number_kfolds))

rf_all_info=np.empty((number_combo, number_permutation, number_kfolds, length_test,6))
rf_info=np.empty((length_test,6))
rf_RandA=np.empty((number_combo,number_permutation,number_kfolds))
rf_pr_auc=np.empty((number_combo,number_permutation,number_kfolds))
rf_roc_auc=np.empty((number_combo,number_permutation,number_kfolds))
rf_ap=np.empty((number_combo,number_permutation,number_kfolds))
rf_ap_one=np.empty((number_combo,number_permutation,number_kfolds))


rf_p_ts_p=np.empty((number_combo,number_permutation))
rf_p_tp_p=np.empty((number_combo,number_permutation))
rf_p_fn_p=np.empty((number_combo,number_permutation))
rf_p_fp_p=np.empty((number_combo,number_permutation))
rf_p_tn_p=np.empty((number_combo,number_permutation))
rf_p_RandA=np.empty((number_combo,number_permutation))
rf_p_pr_auc=np.empty((number_combo,number_permutation))
rf_p_roc_auc=np.empty((number_combo,number_permutation))
rf_p_ap=np.empty((number_combo,number_permutation))
rf_p_ap_one=np.empty((number_combo,number_permutation))


rf_pre=np.empty((length_test,number_combo,number_permutation,number_kfolds ))


#XGB

xgb_fold_ts_k=np.empty((number_combo,number_permutation,number_kfolds ))
xgb_fold_tp_k=np.empty((number_combo,number_permutation,number_kfolds))
xgb_fold_fn_k=np.empty((number_combo,number_permutation,number_kfolds))
xgb_fold_fp_k=np.empty((number_combo,number_permutation,number_kfolds))
xgb_fold_tn_k=np.empty((number_combo,number_permutation,number_kfolds))


xgb_all_info=np.empty((number_combo, number_permutation, number_kfolds, length_test,6))
xgb_info=np.empty((length_test,6))
xgb_RandA=np.empty((number_combo,number_permutation,number_kfolds))
xgb_pr_auc=np.empty((number_combo,number_permutation,number_kfolds))
xgb_roc_auc=np.empty((number_combo,number_permutation,number_kfolds))
xgb_ap=np.empty((number_combo,number_permutation,number_kfolds))
xgb_ap_one=np.empty((number_combo,number_permutation,number_kfolds))


xgb_p_ts_p=np.empty((number_combo,number_permutation))
xgb_p_tp_p=np.empty((number_combo,number_permutation))
xgb_p_fn_p=np.empty((number_combo,number_permutation))
xgb_p_fp_p=np.empty((number_combo,number_permutation))
xgb_p_tn_p=np.empty((number_combo,number_permutation))
xgb_p_RandA=np.empty((number_combo,number_permutation))
xgb_p_pr_auc=np.empty((number_combo,number_permutation))
xgb_p_roc_auc=np.empty((number_combo,number_permutation))
xgb_p_ap=np.empty((number_combo,number_permutation))
xgb_p_ap_one=np.empty((number_combo,number_permutation))


xgb_pre=np.empty((length_test,number_combo,number_permutation,number_kfolds ))




lm_ra=np.empty((number_combo,1))
lm_roc_auc_m=np.empty((number_combo,1))
lm_ap_m=np.empty((number_combo,1))
lm_ap_one_m=np.empty((number_combo,1))
lm_ts_median=np.empty((number_combo,1)) 
lm_ts_mean=np.empty((number_combo,1))
lm_ts_m=np.empty((number_combo,3)) 
lm_tp_m=np.empty((number_combo,1))
lm_fp_m=np.empty((number_combo,1))
lm_fn_m=np.empty((number_combo,1))
lm_tn_m=np.empty((number_combo,1))
lm_pi_m=np.empty((number_combo,1))
lm_positive_predictive_value_or_precision=np.empty((number_combo,1))
lm_false_positive_rate=np.empty((number_combo,1))
lm_true_negative_rate_or_specificity=np.empty((number_combo,1))
lm_positive_predictive_value=np.empty((number_combo,1))
lm_negative_predicted_value=np.empty((number_combo,1))
lm_recall_or_sensitivity_or_true_positive_rate=np.empty((number_combo,1))
lm_f1_score=np.empty((number_combo,1))
lm_ts_median_diviation_fold= np.empty((number_combo,1))  
lm_pr2= np.zeros((number_combo,1)) 


#LDA
lda_ra=np.empty((number_combo,1))
lda_roc_auc_m=np.empty((number_combo,1))
lda_ap_m=np.empty((number_combo,1))
lda_ap_one_m=np.empty((number_combo,1))
lda_ts_median=np.empty((number_combo,1)) 
lda_ts_mean=np.empty((number_combo,1))
lda_ts_m=np.empty((number_combo,3)) 
lda_tp_m=np.empty((number_combo,1))
lda_fp_m=np.empty((number_combo,1))
lda_fn_m=np.empty((number_combo,1))
lda_tn_m=np.empty((number_combo,1))
lda_pi_m=np.empty((number_combo,1))
lda_positive_predictive_value_or_precision=np.empty((number_combo,1))
lda_false_positive_rate=np.empty((number_combo,1))
lda_true_negative_rate_or_specificity=np.empty((number_combo,1))
lda_positive_predictive_value=np.empty((number_combo,1))
lda_negative_predicted_value=np.empty((number_combo,1))
lda_recall_or_sensitivity_or_true_positive_rate=np.empty((number_combo,1))
lda_f1_score=np.empty((number_combo,1))
lda_ts_median_diviation_fold= np.empty((number_combo,1))  
lda_pr2= np.zeros((number_combo,1))

#RF
rf_ra=np.empty((number_combo,1))
rf_roc_auc_m=np.empty((number_combo,1))
rf_ap_m=np.empty((number_combo,1))
rf_ap_one_m=np.empty((number_combo,1))
rf_ts_median=np.empty((number_combo,1)) 
rf_ts_mean=np.empty((number_combo,1))
rf_ts_m=np.empty((number_combo,3)) 
rf_tp_m=np.empty((number_combo,1))
rf_fp_m=np.empty((number_combo,1))
rf_fn_m=np.empty((number_combo,1))
rf_tn_m=np.empty((number_combo,1))
rf_pi_m=np.empty((number_combo,1))
rf_positive_predictive_value_or_precision=np.empty((number_combo,1))
rf_false_positive_rate=np.empty((number_combo,1))
rf_true_negative_rate_or_specificity=np.empty((number_combo,1))
rf_positive_predictive_value=np.empty((number_combo,1))
rf_negative_predicted_value=np.empty((number_combo,1))
rf_recall_or_sensitivity_or_true_positive_rate=np.empty((number_combo,1))
rf_f1_score=np.empty((number_combo,1))
rf_ts_median_diviation_fold= np.empty((number_combo,1))  
rf_pr2= np.zeros((number_combo,1))

#XGB
xgb_ra=np.empty((number_combo,1))
xgb_roc_auc_m=np.empty((number_combo,1))
xgb_ap_m=np.empty((number_combo,1))
xgb_ap_one_m=np.empty((number_combo,1))
xgb_ts_median=np.empty((number_combo,1)) 
xgb_ts_mean=np.empty((number_combo,1))
xgb_ts_m=np.empty((number_combo,3)) 
xgb_tp_m=np.empty((number_combo,1))
xgb_fp_m=np.empty((number_combo,1))
xgb_fn_m=np.empty((number_combo,1))
xgb_tn_m=np.empty((number_combo,1))
xgb_pi_m=np.empty((number_combo,1))
xgb_positive_predictive_value_or_precision=np.empty((number_combo,1))
xgb_false_positive_rate=np.empty((number_combo,1))
xgb_true_negative_rate_or_specificity=np.empty((number_combo,1))
xgb_positive_predictive_value=np.empty((number_combo,1))
xgb_negative_predicted_value=np.empty((number_combo,1))
xgb_recall_or_sensitivity_or_true_positive_rate=np.empty((number_combo,1))
xgb_f1_score=np.empty((number_combo,1))
xgb_ts_median_diviation_fold= np.empty((number_combo,1))  
xgb_pr2= np.zeros((number_combo,1))
...
#Model weights ste for sqrt 
#model = LogisticRegression(penalty=None, solver = 'newton-cg', max_iter= 150, class_weight={0:1, 1:np.sqrt(no_train/yes_train)})
...
xgb_model = xgb.XGBClassifier(
    ...
    monotone_constraints=(1,1,1),  # must match X’s column order/length
    random_state=0,
    n_jobs=-1
)
